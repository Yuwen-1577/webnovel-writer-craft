...
Dashboard 应读取以下文件展示叙事质量趋势（由 review skill 生成）：

| 数据源 | 文件路径 | 内容 |
|--------|---------|------|
| 叙事评分历史 | `${PROJECT_ROOT}/.webnovel/metrics/narrative_scores.jsonl` | 每章 5 维度评分（价值转变/冲突有效性/弧光一致性/风格一致性/伏笔张力） |
| 最新评分 | `${PROJECT_ROOT}/.webnovel/tmp/review_narrative_scores.json` | 最近一次审查的评分详情 |
| 风格配置 | `${PROJECT_ROOT}/.webnovel/state.json` → `project.writing_style` | 当前选定风格流派+参数 |
| 风格圣经 | `${PROJECT_ROOT}/设定集/风格圣经.md` | 完整风格指引文档 |
