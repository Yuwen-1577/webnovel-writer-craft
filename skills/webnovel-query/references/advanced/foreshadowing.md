---
name: foreshadowing
purpose: 伏笔紧急度分析和管理时加载
---

<context>
此文件用于伏笔管理参考。Claude 已知一般叙事伏笔技巧，这里补充网文特定的伏笔层级、回收周期、紧急度计算，以及叙事张力理论基础。
</context>

<instructions>

## 叙事张力理论基础

### 契诃夫之枪原则

"如果第一幕墙上挂了一把枪，第三幕它必须开火。"

核心：每个出现的元素都必须有回报。反之，每个回报都必须有铺垫。没有无因之果，也没有无果之因。

### 悬念 vs 惊奇（Hitchcock）

| 类型 | 定义 | 读者体验 | 效果 |
|------|------|---------|------|
| **悬念** | 读者知道危险，角色不知道 | "桌下有炸弹！快跑啊！" | 持续紧张 |
| **惊奇** | 读者和角色都不知道 | "砰！炸弹爆炸了！" | 短暂震撼 |

**原则**：用悬念 > 用惊奇。伏笔制造悬念，反转制造惊奇。最佳组合：先悬念后惊奇。

### 五种伏笔模式

| 模式 | 原理 | 示例 |
|------|------|------|
| **顺手埋** | 正常剧情中顺带一提 | 翻书时看到"吞天魔功"传说 |
| **对话埋** | 角色对话透露 | "你的体质很特殊..." |
| **细节埋** | 不起眼的细节 | 破庙神像缺了一只手臂 |
| **梦境埋** | 梦境暗示未来 | 梦到血色世界和模糊身影 |
| **草蛇灰线** | 多次出现但不点明 | 每次出场都留悬念，但不明说 |

---

## 伏笔层级分类

| 层级 | 回收周期 | 权重 | 示例 |
|------|---------|------|------|
| **核心** | 50-300章 | 3.0x | 主角身世、终极敌人、金手指来历 |
| **支线** | 30-100章 | 2.0x | 配角动机、神秘物品、师门秘密 |
| **装饰** | 10-30章 | 1.0x | 场景细节、小习惯、路人提及 |

## 紧急度计算

```
紧急度 = (已过章节 / 目标章节) × 层级权重
```

**状态判定**:
- 🔴 Critical: 超过目标章节 OR 核心伏笔 >50 章未回收
- 🟡 Warning: >80% 目标 OR 支线 >80 章未回收
- 🟢 Normal: 在计划范围内

## 伏笔埋设技巧

| 技巧 | 方法 | 示例 |
|------|------|------|
| 顺手埋 | 正常剧情中顺带一提 | 藏书阁翻书时看到"吞天魔功"传说 |
| 对话埋 | 通过角色对话透露 | "你的体质很特殊..." |
| 细节埋 | 不起眼的细节 | 破庙神像缺了一只手臂 |
| 梦境埋 | 通过梦境暗示未来 | 梦到血色世界和模糊身影 |

## 伏笔回收方式

| 方式 | 适用层级 | 效果 |
|------|---------|------|
| 直接揭晓 | 浅层 | 快速交代 |
| 层层揭开 | 中层 | 逐步揭示真相 |
| 意外反转 | 深层 | 制造惊喜震撼 |

## 伏笔密度建议

| 作品长度 | 浅层 | 中层 | 深层 |
|---------|------|------|------|
| 短篇(10万字) | 5-10条 | 2-3条 | 1条 |
| 中篇(50万字) | 20-30条 | 5-10条 | 2-3条 |
| 长篇(200万字+) | 50+条 | 20+条 | 5-10条 |

**原则**: 同时进行的伏笔不超过 5 条

## 禁忌

- 埋了不收（读者期待落空）
- 强行圆回（明显是后补的）
- 伏笔太明显（一眼看穿）
- 伏笔过多（读者记不住）

</instructions>

<examples>

<example>
<input>设计主角身世伏笔（核心层级）</input>
<output>
**第 1 章埋**: 主角父亲神秘失踪
**第 50 章线索**: 发现父亲留下的信，提到北域
**第 100 章线索**: 在北域找到父亲踪迹
**第 200 章回收**: 父亲现身，原来是为了封印魔神！

**回收方式**: 层层揭开
**埋设技巧**: 对话埋（母亲临终遗言）+ 细节埋（父亲留下的玉佩）
</output>
</example>

<example>
<input>伏笔紧急度分析</input>
<output>
**当前章节**: 80
**伏笔 F-001**: 神秘玉佩（核心，target=50）
  紧急度 = (80/50) × 3.0 = 4.8
  状态: 🔴 Critical（已超过目标！）

**伏笔 F-002**: 师妹身份（支线，target=100）
  紧急度 = (80/100) × 2.0 = 1.6
  状态: 🟡 Warning（>80%）

**伏笔 F-003**: 破庙细节（装饰，target=120）
  紧急度 = (80/120) × 1.0 = 0.67
  状态: 🟢 Normal
</output>
</example>

<example>
<input>草蛇灰线设计</input>
<output>
**第 10 章**: 主角遇到乞丐老者，给了他一颗丹药
**第 30 章**: 主角又遇到这个老者，老者笑而不语
**第 50 章**: 关键时刻，老者出手相救
**第 100 章**: 老者身份揭晓——上古大能！

**技巧**: 每次出现都留悬念，但不明说
</output>
</example>

</examples>

<errors>
❌ 埋了不收（读者骂坑） → ✅ 建立伏笔清单，定期回收
❌ 强行圆回（牵强附会） → ✅ 埋得越早，回收越要合理
❌ 伏笔太明显（一眼看穿） → ✅ 若隐若现，不要太直白
❌ 同时 10+ 条伏笔 → ✅ 同时进行不超过 5 条
❌ 核心伏笔 >50 章不回收 → ✅ 检查紧急度，及时回收
</errors>
