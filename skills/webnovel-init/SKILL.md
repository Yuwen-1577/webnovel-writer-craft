...
| Step 2 | always | 叙事结构选型 | `${SKILL_ROOT}/../../references/shared/narrative-structures.md` |
| Step 2 | always | 写作风格选型 | `${SKILL_ROOT}/../../references/shared/style-library.md` |
| Step 2 | always | 题材风格桥接 | `${SKILL_ROOT}/../../references/shared/genre-style-bridge.md` |
| Step 3 | always | 角色心理学 | `${SKILL_ROOT}/../../references/shared/character-psychology.md` |
...
- 核心冲突
- 目标读者/平台
- **叙事结构选型**：从六大框架中选择主框架+辅助框架（详见 `references/shared/narrative-structures.md`）。基于题材和目标读者推荐 2-3 个候选组合，用户选择或自定义
- **写作风格选型**：从风格库中选择主风格流派+子变体（详见 `references/shared/style-library.md`）。基于题材和叙事结构推荐 2-3 个候选风格，用户选择或自定义。同时选定风格参数的默认值（句长/白描比例/对话权重/节奏/情感温度）
...
- 感情线配置（无/单女主/多女主）
- 反派分层（小/中/大）与镜像对抗一句话
- **角色弧光类型**（必收）：正向弧（克服缺陷）/负向弧（被缺陷吞噬）/平坦弧（改变世界），详见 `references/shared/character-psychology.md`

收集项（可选）：
- 主角原型标签（成长型/复仇型/天才流等）
- **主角心理原型**（可选）：从 Jung 12 原型中选择，辅助性格生成（详见 `references/shared/character-psychology.md`）
- 多主角分工
...
```json
{
  ...
  "constraints_inherited": {
    ...
    "antagonist_mirror": "",
    "opening_hook": "",
    "narrative_structure_primary": "",
    "narrative_structure_secondary": ""
  }
}
```
...
基于 Step 2 选定的写作风格，自动生成 `${PROJECT_ROOT}/设定集/风格圣经.md`：

1. 从 `${SKILL_ROOT}/../../references/shared/style-bible-template.md` 读取模板
2. 从 `${SKILL_ROOT}/../../references/shared/style-library.md` 读取选定流派/子变体的完整作者档案
3. 从 `${SKILL_ROOT}/../../references/shared/genre-style-bridge.md` 读取题材约束（爽点密度、压扬比例）
4. 填充模板中所有 `{placeholder}` 字段
5. 写入 `${PROJECT_ROOT}/设定集/风格圣经.md`

此文件是 write 阶段 context-agent 的风格权威来源，不再每次从 style-library.md 重新检索。
