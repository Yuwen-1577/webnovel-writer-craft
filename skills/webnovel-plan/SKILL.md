...
| Step 6 | always | `../../references/shared/narrative-structures.md` |
| Step 7 | always | `../../references/shared/scene-value-change.md` |
...
| Step 6/7 | 需要剧情框架模板 | `references/outlining/plot-frameworks.md` |
...
- 阻力
- 代价
- **本章价值转变**（必填）：开始时的核心价值状态 → 结束时的状态（如"安全→危险"、"无知→知晓"、"孤立→联盟"），详见 `references/shared/scene-value-change.md`
- **冲突类型**（必填）：从六种冲突子类型中选择（人vs人/自然/社会/自我/技术/命运）
- **本章风格**（必填）：从 state.json 的 writing_style 读取默认风格，若本章需要切换则标注切换原因和目标风格（详见 `references/shared/style-library.md` 的风格切换操作指南）
- 时间锚点
- 章内时间跨度
- 与上章时间差
- 倒计时状态
...
```json
{
  ...
  "narrative_structure": {
    "primary": "本卷使用的主叙事结构（如 Save the Cat / 英雄之旅 / 起承转合）",
    "beat_mapping": "本卷在主结构中的位置（如 STC 的 0-100% 对应）"
  },
