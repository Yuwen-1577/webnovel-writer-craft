...
Agent(
  ...
  prompt: "chapter={chapter_num}; project_root=${PROJECT_ROOT}; scripts_dir=${SCRIPTS_DIR}; storage_path=${PROJECT_ROOT}/.webnovel; state_file=${PROJECT_ROOT}/.webnovel/state.json（projection/read-model，仅兼容读取）。先 research，再按 本章硬性约束→CBN/CPNs/CEN→本章禁区→风格指引→dynamic_context补充参考 的顺序输出五段写作任务书。风格指引必须包含：从 state.json 读取 writing_style 字段（流派+子变体+参考作者+参数），并加载 references/shared/style-library.md 中对应风格档案的详细描述（句式特征、描写密度、对话风格、节奏参数）。"
)
```
...
| 触发条件 | Reference |
|---------|-----------|
| 需要视角/对话/节奏技法 | `../../references/shared/writing-craft.md` |
| 需要选择/切换写作风格 | `../../references/shared/style-library.md` |
| 需要场景价值转变检查 | `../../references/shared/scene-value-change.md` |
| 需要风格适配 | `${SKILL_ROOT}/references/style-adapter.md` |
| 需要特定题材写法 | CSV 检索 `--table 场景写法` |
