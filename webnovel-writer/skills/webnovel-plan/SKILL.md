...
| Step 6 always | 全文 | `${SKILL_ROOT}/../../references/shared/narrative-structures.md` |
| Step 7 always | 全文 | `${SKILL_ROOT}/../../references/shared/scene-value-change.md` |
| Step 7 always | 全文 | `${SKILL_ROOT}/../../references/shared/style-system.md` |
...
每章必须包含：目标、阻力、代价、**本章价值转变**（开始时核心价值状态→结束时状态，详见 `references/shared/scene-value-change.md`）、**冲突类型**（人vs人/自然/社会/自我/技术/命运）、**本章风格**（默认继承 state.json writing_style，需切换则标注理由，详见 `references/shared/style-system.md`）、时间锚点、章内时间跨度、与上章时间差、倒计时状态、爽点、Strand、反派层级、视角/主角、关键实体、本章变化、章末未闭合问题、钩子，以及结构化节点 `CBN`、`CPNs`、`CEN`、`必须覆盖节点`、`本章禁区`。
