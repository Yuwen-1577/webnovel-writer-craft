...
2. 解析用户输入（`/webnovel-learn` 后的经验文本；为空则取本次对话中用户认可的写法），归类 `pattern_type`：
   - 基础类型：hook/pacing/dialogue/payoff/emotion/format
   - 叙事学类型：value_change（价值转变）/conflict_type（冲突模式）/arc_beat（弧光节拍）/style_switch（风格切换）/subtext（潜台词技巧）/foreshadow（伏笔技法）
   - 兜底：other
...
python -X utf8 "${SCRIPTS_DIR}/webnovel.py" --project-root "${PROJECT_ROOT}" project-memory add-pattern \
  ...
  --style-context "{当前写作风格流派+子变体，从 state.json writing_style 读取}"
```
